...
class Network:
    ...
    def __map_identifiers(self) -> None:
        ...
        for index, metadata in self.nodes.items():
            ...
            symbol_ids = self.__reverse_mapper[node_symbol][IDENTIFIERS]
            id_anno = ("", "")  # Initialize as empty
            if symbol_ids is not None:
                if node_molecule == PROTEIN:
                    id_anno = (symbol_ids['uniprot'], "") if symbol_ids['uniprot'] is not None else ("", "")

                elif node_molecule == DNA:
                    hgnc_id = symbol_ids['hgnc'] if symbol_ids['hgnc'] is not None else ""
    ...
    def __get_identifiers(self) -> None:
        ...
        for symbol in tqdm(self.__reverse_mapper.keys(), desc="Gathering identifiers",
                           total=len(self.__reverse_mapper)):
            profiler = Profiler(symbol)

            if not profiler.cache_file_exists():
                sleep(0.1)  # Sleep if it needs to be downloaded

            identifiers = profiler.get_identifers()
    ...
    def import_nodes(self, node_file_path: str) -> None:
        ...
        for line in content:
            ...
            elif len(components) == 5:  # Enriched with molecule and identifiers
                self.nodes[int(components[0])] = {SYMBOL: components[1], MOLECULE: components[2],
                                                  IDENTIFIERS: (components[3], components[4])}
            else:
...
class Analyzer(Network):
    ...
    def shortest_paths(self, source: str, target: str, print_paths: bool = False) -> Optional[list]:
        ...
        if print_paths:
            for single_path in self.paths:
                START, END = "(START) ", " (END)"
                path_symbols = [self.nodes[node_id]['symbol'] for node_id in single_path]
