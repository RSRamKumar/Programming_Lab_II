...
@click.option('-p','--ppi',help='Name of the Protein-Protein Interaction File',type=click.Path(exists=True),required=False)
@click.option('-n','--nodes',help='Name of the Node File',type=click.Path(exists=True), required=False)
@click.option('-e','--edges',help='Name of the Edge File',type=click.Path(exists=True), required=False)
@click.argument('output_file_path')  


def main(create,ppi,nodes,edges,output_file_path):
    """overall click function to generate network graph,
    param create: label of the cli command,
    param ppi: name of the ppi file,
    param nodes : name of the node file,
    param edges : name of the edge file,
    param output_file_path : name of the output file to be generated,
    return graph"""
    def drawing_graph(edge_list_path:str,node_list_path:str,graph_output_path:str)->nx.Graph:
        """a compiling function to generate a graph 
        param edge_list_path: name of the edge file
        param node_list_path: name of the node file
        param graph output path : name of the output file
        return graph"""
        ...
        def import_node_labels(node_file_path:str)->dict:
        ...
        def generate_graph(graph_output_path:str, edge_list:list, node_list:dict)-> nx.Graph:
        ...
        generate_graph(graph_output_path, edge_list_path, node_list_path)
 
     
        
    def output_file_format_check(output_file_path:str)->None:
        """function to check for file format
    ...
    if ppi:
        def reading_the_file(ppi:str)->List[str]:
        ...
        def producing_edge_identifer(input_file_path,node_list):
        ...
        output_file_format_check(output_file_path)
        node_file_path, edge_file_path = "node_list.tsv", "edge_list.tsv"
        compile_function(ppi,node_file_path,edge_file_path)

        drawing_graph(edge_file_path,node_file_path,output_file_path)
        
        
    else:
        drawing_graph( edges, nodes,output_file_path)
